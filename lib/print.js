...
module.exports = async function (m, conn = { user: {} }) {
  if (!WAMessageStubType) {
    const baileysModule = await import('@adiwajshing/baileys')
    WAMessageStubType = baileysModule.WAMessageStubType || baileysModule.default?.WAMessageStubType || baileysModule.proto?.MessageStubType || {}
  }

  let _sender = conn.getJid(m.sender)
  let _name = await conn.getName(_sender)
  let sender = (_sender.endsWith('@s.whatsapp.net') ? PhoneNumber('+' + _sender.replace('@s.whatsapp.net', '')).getNumber('international') : _sender) + (_name ? ' ~' + _name : '')
