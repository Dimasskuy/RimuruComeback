...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.isLid = {
        ...
        get(key) {
            ...
            if (global.db?.data?.isLid?.[key]) {
                const val = global.db.data.isLid[key];
                lidCache.set(key, val);
                return val;
            }
            return null;
        },
        has(key) { return lidCache.has(key) || !!global.db?.data?.isLid?.[key]; },
        delete(key) {
            lidCache.delete(key);
    ...
    conn.getJid = (sender) => {
        ...
        if (!sender || !sender.endsWith('@lid')) return sender;
        const cached = conn.isLid.get(sender);
        if (cached) return cached;

        // Search in conn.chats for mapping
        for (const id in conn.chats) {
            const participants = conn.chats[id]?.metadata?.participants;
            if (participants) {
                for (const p of participants) {
                    const decoded = conn.decodeJid(p.id);
                    if (p.lid === sender || decoded === sender) {
                        if (p.lid && decoded && !decoded.endsWith('@lid')) {
                            conn.isLid.set(p.lid, decoded);
                            return decoded;
                        }
                    }
                }
    ...
    function updateNameToDb(contacts) {
        ...
        for (const contact of contacts) {
            ...
            // Populating LID cache from contact/group updates
            if (contact.participants) {
                for (const p of contact.participants) {
                    const decoded = conn.decodeJid(p.id);
                    if (p.lid) conn.isLid.set(p.lid, decoded);
                }
            }
    ...
    conn.ev.on('groups.upsert', updateNameToDb)
    ...
    conn.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        ...
        if (groupMetadata.participants) {
            for (const p of groupMetadata.participants) {
                p.decodedId = conn.decodeJid(p.id);
                if (p.lid) conn.isLid.set(p.lid, p.decodedId);
            }
        }
        conn.chats[id] = {
            ...conn.chats[id],
...
`.trim()
    ...
    conn.getName = (jid = "", withoutContact = false) => {
        ...
        const contact = conn.chats[jid];
        const name = contact?.name || contact?.verifiedName || contact?.notify;
        if (name && !name.includes('@')) return name;

        const num = PhoneNumber("+" + jid.replace("@s.whatsapp.net", ""));
        if (num.isValid()) return num.getNumber("international");

        const jidNum = jid.split('@')[0];
        if (jidNum.length < 15) return '+' + jidNum; // Likely a phone number
    ...
    conn.insertAllGroup = async() => {
        ...
        for (const group in groups) {
            ...
            conn.chats[group] = { ...(conn.chats[group] || {}), id: group, subject: groupData.subject, isChats: true, metadata: groupData };
        }
        return conn.chats;
    }
    ...
    conn.pushMessage = async(m) => {
        ...
        for (const message of m) {
            try {
                ...
                // Track LID from sender reliably
                const sender = message.key.participant || message.key.remoteJid;
                if (sender && sender.endsWith('@lid')) {
                    // Baileys sometimes provides the JID in message attributes or we find it later
                    // For now, we trust the cache or wait for a group event
                }
                ...
                if (!['protocolMessage', 'senderKeyDistributionMessage'].includes(mtype) && !fromMe && message.messageStubType != WAMessageStubType.CIPHERTEXT && message.message) {
                    chats.messages = chats.messages || {}
                    chats.messages[message.key.id] = JSON.parse(JSON.stringify(message, null, 2))
                    let qChatsMessages = Object.values(chats.messages)
                    if (qChatsMessages.length > 40) {
                        chats.messages = Object.fromEntries(qChatsMessages.slice(-40).map(v => [v.key.id, v]));
                    }
                }
            } catch (e) {
                console.error(e)
...
exports.smsg = (conn, m) => {
    ...
    if (m.key) {
        ...
        m.sender = conn.getJid(m.key.fromMe ? conn.user.id : (m.key.participant || m.key.remoteJid || ''))
