...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.getJid = (sender) => {
        ...
        // If not in cache, try to find it in chats
...
`.trim()
    ...
    conn.insertAllGroup = async() => {
        ...
        for (const group in groups) {
            ...
            // Populate LID cache from participants
            if (groups[group].participants) {
                for (const p of groups[group].participants) {
                    if (p.lid && p.id) {
                        const jid = p.id.split(':')[0] + '@s.whatsapp.net'
                        conn.isLid.set(p.lid, jid)
                    }
                }
            }
        }
    ...
    conn.pushMessage = async(m) => {
        ...
        for (const message of m) {
            try {
                ...
                // Track LID from sender
                if (message.key.participant?.endsWith('@lid') || message.key.remoteJid?.endsWith('@lid')) {
                    const lid = message.key.participant || message.key.remoteJid
                    const jid = (message.key.participant || message.key.remoteJid).split(':')[0] + '@s.whatsapp.net'
                    if (lid.endsWith('@lid')) conn.isLid.set(lid, jid)
                }
