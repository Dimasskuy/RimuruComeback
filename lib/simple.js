...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.getJid = (sender) => {
        ...
        for (const chat of Object.values(conn.chats || {})) {
            ...
            if (user) {
                // Determine JID from available user properties
                let jid = user?.id || user?.jid

                // If the id is still LID but phoneNumber is available, construct the JID
                if (jid?.endsWith('@lid') && user?.phoneNumber) {
                    jid = user.phoneNumber + '@s.whatsapp.net'
                }

                // If we got a valid jid that is NOT a LID, cache and return it
                if (jid && !jid.endsWith('@lid') && jid.includes('@s.whatsapp.net')) {
    ...
    if (m.key) {
        ...
        let senderJid = m.key.fromMe
            ...
            : m.key.participantPn || m.key.senderPn ||
              ...
              || conn.getJid(m.key.participant || m.key.senderLid || m.key.remoteJid || m.participant || m.chat || '')
              || m.key.participant || m.key.senderLid || m.key.remoteJid || m.participant || m.chat || '';

        m.sender = conn.decodeJid(senderJid)

        // Final fallback: if sender is still ending up as a LID, let's strictly re-check PN sources
        if (m.sender.endsWith('@lid')) {
             if (m.key.participantPn && !m.key.participantPn.endsWith('@lid')) {
                 conn.isLid.set(m.sender, m.key.participantPn)
                 m.sender = m.key.participantPn
             } else if (m.key.senderPn && !m.key.senderPn.endsWith('@lid')) {
                 conn.isLid.set(m.sender, m.key.senderPn)
                 m.sender = m.key.senderPn
             }
        }
