...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.getJid = (sender) => {
        ...
        const dbMappedJid = global.db.data?.isLid?.[sender]
        if (dbMappedJid?.endsWith('@s.whatsapp.net')) {
            conn.isLid.set(sender, dbMappedJid)
            return dbMappedJid
        }
    ...
    conn.ev.on('lid-mapping.update', ({ lid, pn }) => {
        const lidJid = conn.decodeJid(lid)
        let pnJid = conn.decodeJid(pn)
        if (pnJid && !pnJid.includes('@')) pnJid = `${pnJid}@s.whatsapp.net`
        if (lidJid?.endsWith('@lid') && pnJid?.endsWith('@s.whatsapp.net')) {
            conn.isLid.set(lidJid, pnJid)
        }
    })
    ...
    if (m.key) {
        ...
        let senderJid = m.key.fromMe
            ...
            : m.key.participantPn || m.key.senderPn ||
              ...
              || conn.getJid(m.key.participant || m.key.senderLid || '')
              || m.key.participant || m.key.senderLid || m.key.remoteJid || m.participant || m.chat || '';

        m.sender = conn.decodeJid(senderJid)
        m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, conn.user.id)

        const participantLid = conn.decodeJid(m.key.participant || m.key.senderLid || '')
        const participantPn = conn.decodeJid(m.key.participantPn || m.key.senderPn || '')
        if (participantLid?.endsWith('@lid') && participantPn) {
            const participantPnJid = participantPn.includes('@') ? participantPn : `${participantPn}@s.whatsapp.net`
            if (participantPnJid.endsWith('@s.whatsapp.net')) {
                conn.isLid.set(participantLid, participantPnJid)
            }
        }
    }
    if (m.message) {
