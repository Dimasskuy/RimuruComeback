...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        if (/image/g.test(mime)) {
            // AI Image logic (using Gemini Vision via Scraping API)
            let img = await q.download?.();
            if (!img) throw "Gagal mendownload gambar";
            let url = await uploadImage(img);
            if (!url) throw "Gagal mengupload gambar";

            let res = await fetch(`https://api.yupra.my.id/api/ai/gemini-vision?q=${encodeURIComponent(text || 'jelaskan gambar ini')}&url=${encodeURIComponent(url)}`);
            if (!res.ok) throw "Gagal menghubungi AI Gambar";
            let json = await res.json();

            if (!json.result) throw "AI tidak memberikan respon untuk gambar ini";

            let result = json.result.trim();
            // Cleanup formatting
            result = result.replace(/\\n/g, "\n").replace(/\n{3,}/g, "\n\n").trim();
            await m.reply(result);
        } else {
            // Text logic (Cici AI logic converted to CJS)
            ...
            if (!response.ok) throw "Server Error (Cici API)";
            ...
            if (!finalResult) throw "AI tidak memberikan respon. Coba lagi nanti.";
            ...
            finalResult = finalResult
                ...
                .replace(/\s{2,}/g, " ")
                .replace(/([.!?])\s*/g, "$1 ")
                .replace(/^\//, '') // Remove potential leading slash
                .trim();
