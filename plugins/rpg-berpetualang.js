...
async function handler(m, { conn, text }) {
    ...
    if (m.sender in conn.adventure) {
        ...
        if (text === 'start') {
            ...
            if (typeof user.loot !== "object") global.db.data.users[m.sender].loot = { potion: 0, diamond: 0, emas: 0, money: 0 };
            ...
            let totalReward = { potion: 0, diamond: 0, emas: 0, money: 0 };
    ...
    if (msg === 'stop') {
        ...
        if (areas[currentArea].txt === msg) {
            ...
            if (currentArea >= areas.length) {
                m.reply(`🎉 Selamat! Anda telah menyelesaikan semua area petualangan.\nTotal hasil petualangan: ${hasilPetualangan}\nExp yang didapatkan: ${reward.exp}\nTotal loot yang didapatkan: Potion: ${totalReward.potion}, Diamond: ${totalReward.diamond}, emas: ${totalReward.emas}, Money: ${totalReward.money}`);
                delete conn.adventure[m.sender];
                return false;
            } else {
