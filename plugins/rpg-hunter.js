...
//  let cds = `${DeTik(new Date - player.lasthunt)}`
 ...
 let timers = clockString(_timers)
  ...
  if (new Date - global.db.data.users[m.sender].lasthunt > 1800000) { // 30 minutes
    let coins = parseInt(Math.floor(Math.random() * 50000) + 25000) // 25K-75K
    let exp = parseInt(Math.floor(Math.random() * 10000) + 5000) // 5K-15K
    let _healing = `${Math.floor(Math.random() * 50) + 25}`.trim() // 25-75 HP
...
Mendapatkan:
${new Intl.NumberFormat('id-ID').format(coins)} Money
