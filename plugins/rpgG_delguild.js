let handler = async (m, { conn, args }) => {
    ...
    let guilds = Object.entries(global.db.data.guilds || {});

    if (guilds.length === 0) return conn.reply(m.chat, 'Tidak ada guild yang tersedia untuk dihapus.', m);

    let guildList = guilds.map(([id, guild], index) => `${index + 1}. ${guild.name}`).join('\n');
    ...
    let [selectedGuildId, selectedGuild] = guilds[guildIndex];
    ...
    delete global.db.data.guilds[selectedGuildId];
